...
- [x] DGP design notebook: `00_dgp_design.ipynb` (synthetic-design reference)
...
- [x] Excel research protocol and compact readout workflow
- [x] Notebook-ready research snippets for data audit, Excel loading, Layer-1 granularity diagnostics, and Layer-2 mean-reversion diagnostics
- [x] Layer-2 mean-reversion starter: levels/changes/squared changes, AR/AR(1), ADF, KPSS, Ljung-Box, residual diagnostics
- [ ] Trading-rule snippet or notebook prototype

## Current focus

Scaffold, parameters, Monte Carlo validation principle, Layer-1 granularity diagnostics, and Layer-2 mean-reversion diagnostics are in place. The canonical empirical workflow is now `research_snippets/`: local Excel workbook in, notebook-ready diagnostics out.

Next: use `research_snippets/03_mean_reversion_starter.py` on the selected 5m data and decide whether the evidence supports a trading-rule prototype. Keep `notebooks/00_dgp_design.ipynb` and `notebooks/01_granularity_selection.ipynb` as non-canonical development references. Do not revive the old combined `notebooks/00_granularity_selection.ipynb`.

## Blocked / open questions

- **Missing-value convention** for real Excel data — audit before final modelling.
- **Anchor choice** for the trading rule (open-of-session recommended as baseline; lock before trading-rule work).
...
- **Resampling convention** — whether each delta-minute candle carries last-in-bin, mean, or log-return sum. Decide in the Layer-1 methodology session and document in `DATA_CONTRACT.md`.
